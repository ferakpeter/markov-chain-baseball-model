...
exports.model = function (assumptions, outTransitions) {
  var atbatProbability = function(transitions, gameState, expectedGameState) {
    ...
    var resultingState = empty(possibleStates) ? 0.0 : possibleStates.filter(r => r.from === gameState.runners && r.to === expectedGameState.runners).map(r => r.transition)[0];
    return empty(resultingState) ? 0.0 : resultingState;
  };
